...
다만 한 가지 불편했던 점은 maxHeight 값이 필수로 입력되어야 했다는 점입니다. 이 값을 auto??로 설정할 수 있어, 이미지의 높이가 비율에 맞춰 자동으로 조정되었으면 더욱 유연하게 사용할 수 있었을 것 같습니다. 특히, 모든 이미지의 높이를 일일이 지정하지 않고 리사이징이 자동으로 이루어졌다면 작업 효율이 더 높아졌을 것입니다.
