...
document.addEventListener("DOMContentLoaded", function () {
  ...
  themeButton.forEach((btn) => {
    btn.addEventListener("click", function () {
      ...
      const codeblocks =
        innerContent != null ? innerContent.querySelectorAll("pre") : null;
  ...
  if (cancelButton) {
    cancelButton.addEventListener("click", function () {
      ...
      Array.from(document.querySelectorAll(".result-item")).forEach(function (
        item
      ) {
        item.remove();
      });
  ...
  document
    ...
    .addEventListener("keyup", function () {
      ...
      if (matchedPosts.length === 0) {
        insertItem(
          '<span class="description">There is no search result.</span>'
        );
      ...
      for (var i = 0; i < matchedPosts.length; i++) {
        ...
        if (highlighted_path === "") highlighted_path = "Home";

        if (matchedPosts[i].type === "post") {
          var highlighted_title = highlightKeyword(
            matchedPosts[i].title,
            keyword
          );
          var highlighted_tags = highlightKeyword(
            matchedPosts[i].tags,
            keyword
          );

          if (highlighted_tags === "") highlighted_tags = "none";

          insertItem(
            '<a href="' +
              matchedPosts[i].url +
              '"><table><thead><tr><th><svg class="ico-book"></svg></th><th>' +
              highlighted_title +
              '</th></tr></thead><tbody><tr><td><svg class="ico-folder"></svg></td><td>' +
              highlighted_path +
              '</td></tr><tr><td><svg class="ico-tags"></svg></td><td>' +
              highlighted_tags +
              '</td></tr><tr><td><svg class="ico-calendar"></svg></td><td>' +
              matchedPosts[i].date +
              "</td></tr></tbody></table></a>"
          );
        } else {
          insertItem(
            '<a href="' +
              matchedPosts[i].url +
              '"><table><thead><tr><th><svg class="ico-folder"></svg></th><th>' +
              highlighted_path +
              "</th></tr></thead></table></a>"
          );
        }
